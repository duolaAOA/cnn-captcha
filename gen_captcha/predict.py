...
def hack_function(sess, predict, captcha_image):
    ...
    :return:
    """
    text_list = sess.run(predict, feed_dict={settings.X: [captcha_image], settings.keep_prob: 1})
...
class HackCaptcha:
    ...
    def save_predict_res(line):
        ...
        with open(arg_settings["predict_img_path"], 'a+', encoding='utf-8') as writer:
            writer.write(line + "\n")
    ...
    def hack(self):
        ...
        with tf.Session() as sess:
            ...
            if checkpoint and checkpoint.model_checkpoint_path:
                saver.restore(sess, tf.train.latest_checkpoint(arg_settings["model_save_path"]))
            ...
            res = 'task:', wrong_count, ' cost time:', (time.time() - stime), 's'
